        ...
        <q-dialog v-model="showUserAdd">
            <q-card>
                ...
                <q-card-section>
                    ...
                    <div class="row q-mb-md">
                        <q-input dense style="min-width: 500px" v-model="activeUser.lastName" label="Last Name" @keyup="updateDisplayName"
...
function loadUsers () {
    ...
    api.get('/users')
        ...
        .catch((error) => notifyError('Unable to load users', error))
        .finally(() => {
            loading.value = false
        })
}
...
function updateDisplayName () {
    activeUser.value.displayName = `${user.value.firstName} ${user.value.lastName}`
}

function createUser () {
    showUserAdd.value = false

    api.post('/users', activeUser)
        .then(() => loadUsers())
        .catch((error) => notifyError('Unable to create user', error))
}
...
function deleteUser (name, id) {
    $q.dialog({
        ...
        api.delete(`/users/${id}`)
            .then(() => loadUsers())
            .catch((error) => notifyError('Failed to delete user', error))
    })
}
